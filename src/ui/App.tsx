...
import { SyntheticMarker } from './primitives';
...
export function App() {
  ...
  const shared = { config, setConfig, derived, onComplete: complete, completed };
  ...
  return (
    <div className="app">
      ...
      <nav className="cases" aria-label="Cases">
        {CASES.map((c) => {
          ...
          return (
            ...
            >
              <span className="cases__index">{c.index === 6 ? '—' : c.index}</span>
              {c.title}
            </button>
          );
        })}
      ...
      <footer className="footer">
        <span className="note">
          Population of {config.size.toLocaleString('en')} generated from seed {config.seed}. Nothing
          leaves this device: no network requests, no analytics, no storage of anything you do beyond
          your case progress.
        </span>
      </footer>
