...
function colourFor(size: number): string {
  const exposure = exposureOf(size);
  if (exposure === 'exposed') return '#a8452c';
  if (exposure === 'narrow') return '#c08a3e';
  return '#191c18';
}
...
export function Field({
  ...
  const draw = useCallback(
    (progress: number) => {
      ...
      // One pass, grouped by colour so fillStyle changes three times rather than n.
      const buckets: Array<{ colour: string; indices: number[] }> = [
        { colour: '#191c18', indices: [] },
        { colour: '#c08a3e', indices: [] },
        { colour: '#a8452c', indices: [] },
      ];
      const bucketOf = new Map(buckets.map((b, i) => [b.colour, i]));

      for (let i = 0; i < layout.x.length; i++) {
        const size = set.classes[layout.classOf[i]].members.length;
        buckets[bucketOf.get(colourFor(size))!].indices.push(i);
      }
    ...
    [layout, set, width, height, zoom, selectedClass],
  );
  ...
  return (
    <div ref={wrapRef} className="field" style={{ height }}>
      <canvas
        ...
        role="img"
        aria-label={`Field of ${recordCount} records in ${set.classes.length} equivalence classes. Smallest class ${set.k}. ${set.singletons} records stand alone.`}
      />
    </div>
  );
}
